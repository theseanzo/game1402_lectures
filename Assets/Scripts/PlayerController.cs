...
public class PlayerController : Unit //this is code for the bear that we control with player input
{
    ...
    private const float DISTANCE_LASER_IF_NO_HIT = 300.0f;
    ...
    { //this is where the magic happens for movement
        ...
        {
            //do our laser shooting code
            Ray ray = new Ray(playerCam.transform.position, playerCam.transform.forward); //we are going to be aiming our ray in the direction we are looking
            RaycastHit hit;
            LayerMask mask = ~LayerMask.GetMask("Outpost","Teddy", "Terrain");
            if(Physics.Raycast(ray, out hit, Mathf.Infinity, mask))
            {
                Debug.Log("we hit something and that something is " + hit.transform);
                if(CanSee(hit.transform, hit.point))
                {
                    //we are going to shoot at our target
                    //Insert ShootAt code here
            ...
            else
            {
                ...
                Vector3 targetPos = playerCam.transform.position + playerCam.transform.forward * DISTANCE_LASER_IF_NO_HIT; //how far to go in distance without hitting
